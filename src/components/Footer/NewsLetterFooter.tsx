export default function NewsletterSection() {
  return (
    <div className="mx-auto text-center mb-12 px-4 sm:px-6 lg:px-16">
      <div className="flex flex-col sm:flex-row justify-between items-center gap-8 sm:gap-[271px]">
        <div className="flex flex-col text-center sm:text-left">
          <h2 className="text-3xl font-bold mb-4">
            <span className="text-[#FF9F0D]">Still</span> You Need Our Support?
          </h2>
          <p className="mb-6 text-lg">
            Don{`'`}t wait, make a smart & logical choice here. It{`'`}s pretty easy.
          </p>
        </div>

        <form className="w-full max-w-lg flex flex-col sm:flex-row gap-4 sm:gap-0 mx-auto">
          <input
            type="email"
            placeholder="Enter Your Email"
            className="flex-grow bg-[#FF9F0D] text-white placeholder-white placeholder-opacity-60 px-4 py-2 rounded-l-md focus:outline-none"
          />
          <button
            type="submit"
            className="bg-white text-[#FF9F0D] px-6 py-2 rounded-r-md font-semibold hover:bg-[#FF9F0D] hover:text-white transition duration-300"
          >
            Subscribe Now
          </button>
        </form>
      </div>
    </div>
  );
}
